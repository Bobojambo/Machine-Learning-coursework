...
"""
Created on Wed Feb  7 17:14:32 2018
...
def grad(w, X, y):
    ...
    for n in range(X.shape[0]):
        ...
        numerator = - np.exp(-y[n]*np.dot(X[n], w)) * y[n]*X[n]
        denominator = 1 + np.exp(-y[n] * np.dot(X[n], w))
...
if __name__ == "__main__":
    ...
    for iteration in range(100):
        ...
        grad_value = grad(w, X, y)
        w = w - step_size*grad_value
